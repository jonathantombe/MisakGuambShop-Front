...
const AdminDashboard = () => {
  ...
  const fetchPendingProducts = async () => {
    try {
      ...
      setError(
        'Error al cargar los productos pendientes. Por favor, intente de nuevo más tarde.'
      )
      setProducts([])
    } finally {
      setLoading(false)
  ...
  const confirmAction = async () => {
    try {
      ...
      setError(
        `Error al ${
          action === 'approved' ? 'aprobar' : 'rechazar'
        } el producto. Por favor, intente de nuevo.`
      )
    }
  }
  ...
  const handleBulkAction = async (bulkAction) => {
    try {
      ...
      setError(
        `Error al realizar la acción masiva. Por favor, intente de nuevo.`
      )
    }
  }
    ...
    <div className="admin-dashboard-container">
      <div className="admin-dashboard-profile-header">
        <div className="admin-dashboard-user-profile-icons">
          <img
            src={getProfileImage()}
            alt={`Perfil de ${user ? user.name : 'Usuario'}`}
            className="admin-dashboard-profile-icon"
          />
          <span className="admin-dashboard-user-name">
            {user ? user.name : ''}
          </span>
          <span>
            <Link
              to="/user/profile/edit"
              className="admin-dashboard-edit-profile"
            >
              Editar perfil
            </Link>
          </span>
      ...
      {showModal && (
        <div className="modal">
          <div className="modal-content">
            <h2>
              {action === 'approved' ? 'Aprobar Producto' : 'Rechazar Producto'}
            </h2>
            <p>
              ¿Está seguro que desea{' '}
              {action === 'approved' ? 'aprobar' : 'rechazar'} este producto?
            </p>
