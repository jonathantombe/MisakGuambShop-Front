...
const MyPublications = () => {
  ...
  const handleAvailabilityChange = (product, newStatus) => {
    setCurrentProduct(product)
    setAction(newStatus)
    setShowModal(true)
  }
  ...
  const confirmAction = async () => {
    try {
      if (action === 'delete') {
        ...
        const endpoint =
          action === 'ENABLED'
            ? `/api/products/${currentProduct.id}/enable`
            : `/api/products/${currentProduct.id}/disable`
        ...
        if (response && response.product) {
          ...
          setProducts(
            products.map((p) =>
              p.id === currentProduct.id ? { ...p, status: action } : p
            )
          )
      ...
      setError(
        `Error al ${
          action === 'delete'
            ...
            ? 'habilitar'
            : 'deshabilitar'
        } el producto. Por favor, intente nuevamente.`
      )
    }
  }
            ...
            {products.length > 0 ? (
              ...
              <div className="no-products-message">
                ...
                <p className="no-products-message-p">
                  Es el momento de mostrar el arte y tradición de nuestra
                  comunidad. Comparte tus productos artesanales y textiles
                  únicos en Misak. Sube tus creaciones, personaliza cada detalle
                  y gestiona tu catálogo de manera sencilla. Tu trabajo merece
                  ser visto: conecta con compradores interesados en preservar la
                  riqueza cultural y disfruta de una plataforma diseñada para
                  hacer crecer tu negocio. ¡Empieza a publicar hoy y marca la
                  diferencia!
                </p>
              </div>
            )}
                ...
                <div className="modal-content">
                  ...
                  <p>
                    {action === 'delete'
                      ...
                      : action === 'DISABLED'
                      ? '¿Está seguro que desea deshabilitar este producto?'
