...
const CartPage = () => {
    ...
    const updatedCart = cartItems.map((item) => {
      if (item.id === id) {
        const newQuantity = Math.max(
          1,
          Math.min(item.stock, (item.quantity || 1) + change)
        )
  ...
  const handlePayment = () => {
    if (!window.ePayco) {
      alert(
        'Error al cargar el sistema de pagos. Por favor, intente nuevamente.'
      )
      return
    }
      ...
      <div className="cart-title-section">
        ...
        <div className="confidence-message">
          ...
          <span className="confidence-text">
            <strong>Compra con confianza en Misak</strong> con el programa de
            protección al comprador, aseguramos que recibirás un reembolso
            completo si tu pedido no llega, llega dañado o no coincide con la
            descripción.
          </span>
        </div>
      </div>
              ...
              <div className="summary-row">
                ...
                <div className="summary-amount">
                  <span className="total-price">
                    <span className="cop">$</span> {totalPrice.toLocaleString()}
                  </span>
                </div>
              </div>
              <button
